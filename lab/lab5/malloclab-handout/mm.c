...
#define PACK(size, alloc) \
  ((size) | (alloc))  // 将 size 和 allocated bit 合并为一个字
...
#define FTRP(bp)                       \
  ((char *)(bp) + GET_SIZE(HDRP(bp)) - \
   DSIZE)  // 获得脚部的地址, 与宏定义HDRP有耦合

#define NEXT_BLKP(bp) \
  ((char *)(bp) + GET_SIZE((char *)(bp)-WSIZE))  // 计算后块的地址
#define PREV_BLKP(bp) \
  ((char *)(bp)-GET_SIZE((char *)(bp)-DSIZE))  // 计算前块的地址
...
// check
/*
static void mm_check(int verbose, const char* func);                 //heap
consistency checker static void mm_checkblock(int verbose, const char* func,
void *bp); static int mm_checkheap(int verbose, const char* func);
*/
...
int mm_init(void) {
  ...
  heap_listp += (2 * WSIZE);  // 小技巧：使heap_listp指向下一块,
                              // 即两个序主块中间
...
static void *find_fit(size_t size) {
  void *curbp;
  for (curbp = heap_listp; GET_SIZE(HDRP(curbp)) > 0;
...
static void place(
    char *bp,
    size_t asize) {  // 注意最小块的限制（16B == DSIZE + 2*WSIZE == MINBLOCK）
...
static void *coalesce(void *bp) {
  ...
  PUT(FTRP(bp),
      PACK(size, 0));  // FTRP()与GET_SIZE()有耦合，故此时所用的SIZE已经改变
