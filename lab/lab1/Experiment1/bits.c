...
int isTmax(int x) {
  ...
  // x=x+i;//-1,1111...
  // x=~x;//0,0000...
  // i=!i;//exclude x=0xffff...(x为-1时+1也为0需要排除)
  // x=x+i;//exclude x=0xffff...
  // return !x;
  return !((~x^(x+1)) | !(x+1));//精简代码（x+1=~x）排除x=-1
}
/* 
...
int allOddBits(int x) {
  return !(((x>>24) & (x>>16) & (x>>8) & x & 0xAA) ^ 0xAA);//规则允许使用最大为 0xFF 的整数，因此可使用 & 运算每 8 位合并 x 的所有位，然后使用 ^ 运算判断奇数位是否全为 1。
}
/* 
 * negate - return -x 
 *   Example: negate(1) = -1.
...
int isLessOrEqual(int x, int y) {
  ...
  return (!(x^y)) | ((x&~y)>>31&1) | ((!((x^y)>>31))&((x+(~y+1))>>31));//相等；x符号为1，y符号为0；符号相等，x-y<0;
}
//4
/* 
...
int logicalNeg(int x) {
  return ~(x|(~x+1))>>31&1;//当 x!=0时，x|(−x)的符号位必然为 1。
}
...
int howManyBits(int x) {
  int bit;
  int res = 1;
  x = x ^ (x >> 31);//将负数取反，将问题统一成计算 1 的最高位，利用算术右移即可完成

  bit = !!(x >> 16) << 4;//高十六位是否有1
  res = res + bit;//如果有（至少需要16位），则将原数右移16位
  x = x >> bit;

  bit = !!(x >> 8) << 3;//剩余位高8位是否有1
  res = res + bit;//如果有（至少需要16+8=24位），则右移8位
  x = x >> bit;
